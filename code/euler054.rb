...
hands = input.strip.split(/\s*\n\s*/).map { |n| n.split }

$card_order = %Q{2, 3, 4, 5, 6, 7, 8, 9, T, J, Q, K, A}.split(/, /).reverse

$card_sort = lambda { |a| $card_order.index(a[0]) }

$rank_order = [
  'highest', 'pair-1', 'pair-2', 'three', 'straight', 'flush', 'full', 'four', 'flush-S', 'flush-R'
]


def check_straight(h)

  (0..$card_order.length-5).each do |g|
    return h[0] if $card_order[g..g+4] == h
  end

  return '5' if h == ['2','3','4','5','A']
  return nil
end
...
def check(h)
  ...
  h.each_with_index do |c,i|
    s = c[1]
    suit[s] = 0 if suit[s].nil?
    suit[s] = suit[s] + 1

    v = c[0]
    val[v] = 0 if val[v].nil?
    val[v] = val[v] + 1
  end
  ...
  if mv == 4
    ...
    if !s.nil?
      ...
      return 'flush', val.keys.sort_by(&$card_sort)
    end
  elsif !s.nil?
    return 'straight',[s]
  elsif mv == 3
    if val.value? 2
      return 'full', [val.key(3), val.key(2)]
    else
      return 'three', [val.key(3)]
    end
  elsif mv == 2
    p = val.keys.select{|i| val[i] == 2}.sort_by(&$card_sort)
    return "pair-#{p.size}", p + (val.keys - p).sort_by(&$card_sort)
  end

  return 'highest', val.keys.sort_by(&$card_sort)
...
def rank(h1, h2)
  ...
  t1 = t1.map{|t| $card_order.index(t)}
  t2 = t2.map{|t| $card_order.index(t)}

  if $rank_order.index(r1) > $rank_order.index(r2)
    return 1
  elsif $rank_order.index(r1) < $rank_order.index(r2)
    return 2
  else
...
sample = sample.strip.split(/\s*\n\s*/).map { |n| n.split }

count = 0

hands.each do |s|
  h1, h2 = s[0..4], s[5..-1]
  count += 1 if rank(h1,h2) == 1
end
